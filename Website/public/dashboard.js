      ...
      onAuthStateChanged(auth, async (user) => {
        ...
        if (isAdmin) {
          ...
          document
            .getElementById("export-dropdown-wrap")
      ...
      async function loadHeaderLogo() {
        try {
          ...
          document
            .getElementById("header-logo-placeholder")
        ...
        for (const uid of userKeys) {
          ...
          for (const sKey of sessionKeys) {
            ...
            if (summary) {
              ...
              const d = summary.completed_at
                ? new Date(summary.completed_at)
                : null;
      ...
      function renderSessionPanel(uid) {
        ...
        ${
          ...
          <!-- Accuracy summary -->
          <div class="bg-rice rounded-xl p-4 border border-timber inline-flex gap-6 items-center mt-3">
            <span class="font-heading text-base font-bold ${accColor(itemAcc)}">${correctItems} / ${totalOrders} items correct</span>
            <span class="font-heading text-base font-bold ${accColor(changeAcc)}">Change accuracy: ${changeAcc}%</span>
          </div>
        </div>`
      ...
      window.startEditUsername = function (uid) {
        document
          .getElementById("detail-username-display")
          .classList.add("hidden");
        document
          .getElementById("detail-username-edit")
          .classList.remove("hidden");
      ...
      window.saveUsername = async function (uid) {
        const newName = document
          .getElementById("detail-username-input")
          .value.trim();
        if (!newName) {
      ...
      window.exportCSV = function () {
        ...
        for (const uid of Object.keys(usersData)) {
          ...
          for (const sKey of Object.keys(playerSessions)) {
            ...
            const orderKeys = Object.keys(txn);
            const totalOrders = orderKeys.length;
            let correctOrders = 0,
              correctChange = 0;
            for (const oKey of orderKeys) {
              if (txn[oKey].is_correct_item) correctOrders++;
              if (txn[oKey].is_change_correct) correctChange++;
            }
            const changeAcc =
              totalOrders > 0
                ? Math.round((correctChange / totalOrders) * 100)
                : 0;
            rows.push([
              u.username || "",
              u.email || "",
